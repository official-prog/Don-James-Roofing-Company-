...
const regions = ["Moody", "Waco", "Temple", "Killeen", "Belton", "Hewitt", "Woodway", "McGregor", "Hillsboro", "Corsicana"];
      ...
      <div className="mx-auto max-w-7xl px-6 grid lg:grid-cols-2 gap-16 items-center">
        <div>
          ...
          <h2 className="font-display text-5xl md:text-7xl leading-[0.95] mt-4 text-balance">
            Central Texas, <em className="copper-text not-italic">locally led.</em>
          </h2>
          ...
          <svg viewBox="0 0 400 400" className="w-full h-full">
            <defs>
              <linearGradient id="tx" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor="oklch(0.64 0.13 150)" />
                <stop offset="100%" stopColor="oklch(0.50 0.16 150)" />
              </linearGradient>
              <filter id="glow">
            ...
            <path
              d="M80,80 L260,80 L260,130 L320,140 L340,200 L320,260 L280,320 L240,360 L200,340 L160,360 L130,320 L100,280 L70,240 L60,180 L80,130 Z"
              fill="oklch(0.50 0.16 150 / 0.06)"
              stroke="oklch(0.50 0.16 150 / 0.6)"
              strokeWidth="1"
            />
            {[
              { x: 185, y: 195, label: "Moody" },
              { x: 195, y: 178, label: "Waco" },
              { x: 210, y: 215, label: "Temple" },
              { x: 220, y: 200, label: "Killeen" },
            ].map((p, i) => (
              <g key={p.label}>
                <motion.circle
                  cx={p.x} cy={p.y} r={6}
                  fill="oklch(0.64 0.13 150)"
                ...
                />
                <text x={p.x + 12} y={p.y + 4} fill="oklch(0.96 0.005 150)" fontSize="11" fontFamily="Inter">{p.label}</text>
              </g>
            ))}
          </svg>
        </div>
